...
def finetuneregion(F,orif,samplename):
    ...
    for key,values in a.items():
        ...
        #不可靠
            ...
            if(typecount[un2] <=2 and int(typecount[un2]) < int(typecount[re1])) and typecount[un2]+typecount[re1]<6 and balancedict[key] < 0.03: #2025/02/28 by heh
                judgere.append([posgene,re1])
                storecon.append(g[key] + [re1] + ['6'])
            #print(key,typecount)
            elif(typecount[un2]/(typecount[un2]+typecount[re1]) > 0.3 and balancedict[key] > 0.01):
                judgere.append([posgene,un2])
    ...
    with open(orif,"r") as fgi:
        for i,oris in enumerate(fgi):
            ...
            #breakpoint = json.loads(line[infosi])
            if(keyori in corlabel):
                ori[rei] = corlabel[keyori]
                #fw.write("\t".join(ori[:12]) + "\n")
            if(re.findall("断点外显子支持",ori[infosi]) and ori[tagi] != "wild"):
                #print(ori)
                ori[rei] = "特别可靠"
