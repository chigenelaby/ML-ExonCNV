...
def finetune(args):
	...
	for i,j,k in zip(a,b,c): #返回文件路径，样品名,vcf file
		...
		balance[j] = demergeinfor(i,j, exonmode, version)
		...
		calivariant(os.path.join(path,"regionexonfinetune.txt"),j,k, version)#根据点突变，校正外显子缺失重复单样品
		#print(i,j)
	#print(balance)
	#以先证者角度合并外显子缺失重复
	if(len(a) > 1 and len(b) > 1 and len(a) == len(b)): #输入多个家系成员，需要对校正后的外显子缺失重复合并家系
		...
		modificationrelative(os.path.join(os.path.dirname(a[0]),"format_gene_inforevis.txt"),balance,"format_gene_inforevis1.txt",sexoncv, version)
...
def finetunetrio(args):
	...
	#01/31/2024  相对可靠修改，家系支持，满足外显子个数以及平衡系数，白名单
	modificationrelative(os.path.join(os.path.dirname(a[0]),"format_gene_inforevistrio.txt"),balance,"format_gene_inforevistrio1.txt",sexoncv, version)
...
parser_a.add_argument("-version",help='version', type=str, default='hg19')
...
parser_b.add_argument("-version",help='version', type=str, default='hg19')
